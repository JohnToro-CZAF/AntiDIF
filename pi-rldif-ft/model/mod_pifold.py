...
"""
#n.b. InverseFoldingDiffusionPiFoldModel is model backbone 
# training_step used and where train step needed to update loss and backpro used
#CategoricalDiffuser deals with adding the noise. 
class InverseFoldingDiffusionPiFoldModel(CategoricalDiffuser, nn.Module):
    ...
    def _get_features(self, S, score, X, mask):
        ...
        #added this n.b.
        if isinstance(self.args.network, dict):
            network_virtual_num = self.args.network['virtual_num']
        else:
            network_virtual_num = self.args.network.virtual_num
        if network_virtual_num > 0: #n.b chagned from self.args.network['virtual_num'] > 0
        ...
        if network_virtual_num > 0: #n.b changed from self.args.network['virtual_num'] > 0
            for i in range(self.virtual_atoms.shape[0]):
                # # true atoms
                for j in range(0, i):
                    node_dist.append(
        ...
        if network_virtual_num > 0: # n.b. was self.args.network['virtual_num'] > 0
            for i in range(self.virtual_atoms.shape[0]):
                edge_dist.append(
                    edge_mask_select(
                        _get_rbf(
        ...
        h_V = []
        #n.b. added to deal with changing type of args
        #added this n.b.
        if isinstance(self.args.network, dict):
            args_node_dist = self.args.network['node_dist']
            args_node_angle = self.args.network['node_angle']
            args_node_direct = self.args.network['node_direct']
            args_edge_dist = self.args.network['edge_dist']
            args_edge_angle = self.args.network['edge_angle']
            args_edge_direct = self.args.network['edge_direct']
        else:
            args_node_dist = self.args.network.node_dist
            args_node_angle = self.args.network.node_angle
            args_node_direct = self.args.network.node_direct
        ...
        if args_node_dist: #n.b. was network.node_dist was self.args.network['node_dist']
            h_V.append(V_dist)
        if args_node_angle: #n.b. was self.args.network['node_dist']
            h_V.append(V_angles)
        if args_node_direct: #n.b. was self.args.network['node_dist']
            h_V.append(V_direct)

        h_E = []
        if args_edge_dist: #n.b. was self.args.network['node_dist']
            h_E.append(E_dist)
        if args_edge_angle: #n.b. was self.args.network['node_dist']
            h_E.append(E_angles)
        if args_edge_direct: #n.b. was self.args.network['node_dist']
            h_E.append(E_direct)
